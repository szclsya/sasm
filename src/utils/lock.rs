...
pub fn check_lock(root: &Path) -> Result<Option<u32>> {
    ...
    if lock_path.is_file() {
        let lock_content = std::fs::read_to_string(lock_path).context("Failed to read lock file")?;
        let lock_info: LockInfo = toml::from_str(&lock_content).context("Failed to parse lock file")?;
...
pub fn lock(root: &Path) -> Result<()> {
    ...
    if lock_path.is_file() {
        ...
        file.write(lock_content.as_bytes()).context("Failed to write lock content")?;
        Ok(())
    }
}
