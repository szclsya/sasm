...
unsafe fn search_sse2(haystack: &[u8], needle: &[u8]) -> Option<usize> {
    use std::arch::x86_64::*;
    ...
    /* unsafe */
    {
        ...
        let n_prefix = _mm_set1_epi32(*needle_4b);

        while h_ptr.add(16) <= h_end {
            let h0 = _mm_loadu_si128(h_ptr as *const _);
            let mask0 = _mm_movemask_epi8(_mm_cmpeq_epi32(h0, n_prefix));
            let h1 = _mm_loadu_si128(h_ptr.offset(1) as *const _);
            let mask1 = _mm_movemask_epi8(_mm_cmpeq_epi32(h1, n_prefix));
            let h2 = _mm_loadu_si128(h_ptr.offset(2) as *const _);
            let mask2 = _mm_movemask_epi8(_mm_cmpeq_epi32(h2, n_prefix));
            let h3 = _mm_loadu_si128(h_ptr.offset(3) as *const _);
            let mask3 = _mm_movemask_epi8(_mm_cmpeq_epi32(h3, n_prefix));

            if (mask0 | mask1 | mask2 | mask3) != 0 {
                let current = std::slice::from_raw_parts(h_ptr, 16 + needle.len());
                for i in 0..16 {
...
pub fn search_rk_fast(haystack: &[u8], needle: &[u8]) -> Option<usize> {
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        if is_x86_feature_detected!("avx2") {
            ...
            return unsafe { search_sse2(haystack, needle) };
        }
    }
